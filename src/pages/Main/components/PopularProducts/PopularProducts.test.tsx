...
test('슬라이더가 렌더링된다', () => {
    render(<PopularProducts products={SAMPLE_PRODUCTS} onAddToCart={noop} onReserve={noop} />);
...
test('count prop으로 표시 개수가 제한된다', async () => {
    const user = userEvent.setup();
    render(<PopularProducts products={SAMPLE_PRODUCTS} count={2} onAddToCart={noop} onReserve={noop} />);
...
test('장바구니에 추가 버튼 클릭 시 onAddToCart가 호출된다', async () => {
    const handleAddToCart = vi.fn();
    const user = userEvent.setup();
    render(<PopularProducts products={SAMPLE_PRODUCTS} onAddToCart={handleAddToCart} onReserve={noop} />);
...
test('예약하기 버튼 클릭 시 onReserve가 호출된다', async () => {
    ...
    const user = userEvent.setup();
    render(<PopularProducts products={SAMPLE_PRODUCTS} onAddToCart={noop} onReserve={handleReserve} />);

    await user.click(screen.getByRole('button', { name: '예약하기' }));
    expect(handleReserve).toHaveBeenCalledWith(SAMPLE_PRODUCTS[0], 1);
});

test('슬라이드 이동 후 수량이 1로 초기화된다', async () => {
    const handleAddToCart = vi.fn();
    const user = userEvent.setup();
    render(<PopularProducts products={SAMPLE_PRODUCTS} onAddToCart={handleAddToCart} onReserve={noop} />);
